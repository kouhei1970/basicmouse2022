...
unsigned char smap[16][16]={0};
unsigned char cmap[16][16]={255};
...
void mode0(void)
{
	...
	straight(H_SECTION + 34, speed, speed, acc, 1);

	while(1){
		smap[mouse_x][mouse_y]=1;
		//座標更新（柱と柱のあいだで更新）
		update_mouse_position(mouse_dir);
		//LED_num((map[mouse_x][mouse_y]&0xf0)>>4);
		//マップ更新
		if(smap[mouse_x][mouse_y]==0)
		{
			...
			{
				set_map(mouse_x, mouse_y, mouse_dir, (wall_lf>0)||(wall_rf>0));
			}
			else
			{	//仮想壁挿入
			...
			{
				set_map(mouse_x, mouse_y, (mouse_dir+1)&3, (wall_rs>0));
			}
			else
			{	//仮想壁挿入
			...
			if(get_map(mouse_x, mouse_y, (mouse_dir-1)&3)== 2)
			{
				set_map(mouse_x, mouse_y, (mouse_dir-1)&3, (wall_ls>0));
			}
			else
			{	//仮想壁挿入
				set_map(mouse_x, mouse_y, (mouse_dir-1)&3, 1);
	...
	straight(H_SECTION, speed, 0, acc, 1);
	turn(180);
	//straight(H_SECTION, speed, speed, acc, 0);
	save_map();
}
